...
if platform_os == "unix":
    ...
    if os.path.exists(os.path.join(p_dir, 'photon.py')) == False:
        subprocess.call(f'setx PATH "{path};{p_dir}"')
        subprocess.call(f'setx /M PATH "{path};{p_dir}"')
        print("Successfully installed! Now you can use the photon command!")
    else:
        print("The program has already been installed")
        sys.exit()
    try:
        with open(os.path.join(p_dir, 'photon.py'), 'wt+') as w:
            w.write(code)
    except:pass
    try:
        with open(os.path.join(p_dir, 'photon.bat'), 'wt+') as w:
