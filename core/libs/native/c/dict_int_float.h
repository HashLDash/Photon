typedef struct dict_int_float_entry {
    int prehash;
    int key;
    double val;
} dict_int_float_entry;

typedef struct dict_int_float {
    int len;
    int size;
    int* indices;
    dict_int_float_entry* entries;
} dict_int_float;

double dict_int_float_get(dict_int_float* self, int key) {
    int size = self->size;
    int index = key % size;
...
void dict_int_float_set(dict_int_float* self, int key, double value) {
    ...
    if (dictLen+1 == self->size) {
        ...
        self->entries = realloc(self->entries, sizeof(dict_int_float_entry)*size);
...
void dict_int_float_del(dict_int_float* self, int key) {
    int size = self->size;
    int index = key % size;
    while (self->entries[self->indices[index]].key != key) {
        index = (index + 1) % size;
