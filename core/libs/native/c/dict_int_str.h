typedef struct dict_int_str_entry {
    int prehash;
    int key;
    char* val;
} dict_int_str_entry;

typedef struct dict_int_str {
    int len;
    int size;
    int* indices;
    dict_int_str_entry* entries;
} dict_int_str;

char* dict_int_str_get(dict_int_str* self, int key) {
    int size = self->size;
    int index = key % size;
...
void dict_int_str_set(dict_int_str* self, int key, char* value) {
    ...
    if (dictLen+1 == self->size) {
        ...
        self->entries = realloc(self->entries, sizeof(dict_int_str_entry)*size);
...
void dict_int_str_del(dict_int_str* self, int key) {
    int size = self->size;
    int index = key % size;
    while (self->entries[self->indices[index]].key != key) {
        index = (index + 1) % size;
