...
class Var(Obj):
    def __repr__(self):
        if self.namespace:
            return f'{self.type} {self.namespace}__{self.value}'
...
class Expr(Obj):
    ...
    def process(self):
        ...
        self.value = elements[index].value
        self.type = elements[index].type
...
class Assign(Obj):
    ...
    def __repr__(self):
        if self.inMemory:
            return f'{self.target.value} = {self.value}'
        else:
...
class Function():
    ...
    def __repr__(self):
        ...
        return f'{self.name}({self.args}{separator}{self.kwargs}) {self.code}'
